...
OUTPUT="${1:-$ROOT/dist/yuttfu Calendar.app}"
TEMP_ROOT="$(mktemp -d)"
...
SDKROOT="$SDK_PATH" CLANG_MODULE_CACHE_PATH="$TEMP_ROOT/module-cache" \
  ...
  --disable-sandbox \
  -c release \
  --product yuttfu-calendar-panel

APP="$TEMP_ROOT/yuttfu Calendar.app"
mkdir -p "$APP/Contents/MacOS" "$APP/Contents/Resources"
cp "$TEMP_ROOT/build/release/yuttfu-calendar-panel" "$APP/Contents/MacOS/yuttfu-calendar-panel"
cp "$ROOT/Resources/Info.plist" "$APP/Contents/Info.plist"
/usr/bin/codesign --force --deep --sign - "$APP" >/dev/null

if [[ -e "$OUTPUT" ]]; then
  if [[ "$(/usr/libexec/PlistBuddy -c 'Print :CFBundleIdentifier' "$OUTPUT/Contents/Info.plist" 2>/dev/null || true)" != "com.yuttfu.sketchybar-calendar" ]]; then
